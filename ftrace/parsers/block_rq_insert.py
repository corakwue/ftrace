...
#block_rq_insert: 179,0 WASM () 6455304 + 8 [mmcqd/0]
BlockRQInsertBase = namedtuple(TRACEPOINT,
    [
    'dev_major',
    'dev_minor',
    'sector',
...
class BlockRQInsert(BlockRQInsertBase):
    __slots__ = ()
    def __new__(cls, dev_major, dev_minor, sector, nr_sector, cmd, comm, rwbs):
            ...
            return super(cls, BlockRQInsert).__new__(
                cls,
                dev_major=dev_major,
                dev_minor=dev_minor,
                sector=sector,
...
block_rq_insert_pattern = re.compile(
        r"""
        (?P<dev_major>\d+),
        (?P<dev_minor>\d+)\s+
        (?P<flush>[F]?)
...
@register_parser
def block_rq_insert(payload):
    """Parser for `block_rq_insert` tracepoint"""
    try:
        match = re.match(block_rq_insert_pattern, payload)
        if match:
