...
TRACEPOINT = 'cpufreq_interactive_target'
...
CPUFreqInteractiveTargetBase = namedtuple(TRACEPOINT,
    [
    'cpu', # CPU
    'load', # Load
...
class CPUFreqInteractiveTarget(CPUFreqInteractiveTargetBase):
    __slots__ = ()
    def __new__(cls, cpu, load, cur, actual, targ):
            ...
            return super(cls, CPUFreqInteractiveTarget).__new__(
                cls,
                cpu=cpu,
                load=load,
                cur=cur,
...
cpufreq_interactive_target_pattern = re.compile(
        r"""
        cpu=(?P<cpu>\d+)\s+
        load=(?P<load>\d+)\s+
        cur=(?P<cur>\d+)\s+
