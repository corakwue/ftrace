...
ClusterEnterBase = namedtuple(TRACEPOINT,
    [
    'name',
    'idx',
...
class ClusterEnter(ClusterEnterBase):
    __slots__ = ()
...
@register_parser
def cluster_enter(payload):
    """Parser for `cluster_enter` tracepoint"""
    try:
        match = re.match(cluster_enter_pattern, payload)
        if match:
